...
public static class MissionCommands
{
    public static void Configure()
    {
        ...
        CommandSystem.Register("MissionBoardExport", AccessLevel.Administrator, MissionBoardExport_OnCommand);
        CommandSystem.Register("misboardexport", AccessLevel.Administrator, MissionBoardExport_OnCommand);
        CommandSystem.Register("MissionBoardImport", AccessLevel.Administrator, MissionBoardImport_OnCommand);
        CommandSystem.Register("misboardimport", AccessLevel.Administrator, MissionBoardImport_OnCommand);
    }
    ...
    [Usage("MissionBoardExport [path]")]
    [Aliases("misboardexport")]
    [Description("Exports mission board placements for fresh-server restore.")]
    private static void MissionBoardExport_OnCommand(CommandEventArgs e)
    {
        var path = e.Length > 0 ? e.GetString(0) : null;
        var count = MissionBoardExport.Export(path);
        e.Mobile.SendMessage(0x35, $"Exported {count:N0} mission boards to {MissionBoardExport.ResolveDisplayPath(path)}.");
    }

    [Usage("MissionBoardImport [path]")]
    [Aliases("misboardimport")]
    [Description("Imports mission board placements from export JSON.")]
    private static void MissionBoardImport_OnCommand(CommandEventArgs e)
    {
        var path = e.Length > 0 ? e.GetString(0) : null;
        if (!MissionBoardExport.Import(path, out var created, out var skipped, out var failureReason))
        {
            e.Mobile.SendMessage(0x22, failureReason ?? "Mission board import failed.");
            return;
        }

        e.Mobile.SendMessage(0x35, $"Imported mission boards. Created: {created:N0}. Already present: {skipped:N0}.");
    }
